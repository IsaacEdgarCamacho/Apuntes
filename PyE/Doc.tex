...
\textbf{\large Probabilidad y Estadistica}
...
Sea $\Omega$ el espacio muestral asociado a un experimento aleatorio $\epsilon$ 
una función $X: \Omega \longrightarrow    \mathbb{R} $  es una variable aletaroria y al conjunto
imagen de $X$ le llamamos $R_x$ que si es finito o infinito numerable la llamamo variables aleatoria 
discreta em otro caso le diremos continua.


\[ f(x)= \left\{ \begin{array}{lcc}
                3x^2 &   si  & 0 \leq x \leq 1 \\
                \\ 0  &  si  & x<0 \vee x > 1
                \end{array}
          \right
    \]
Para que sea función de densidad debe cumplir que:
\begin{enumerate}
    \item $ f(x_i) \geq 0, \forall x_i \in \epsilon$
    \item $ \int_{R_x}^{} f(x) \,dx = 1$    
\end{enumerate}
\[
    \int_{R_x}^{} 3x^2 \,dx = 3 \int_{1}^{0} x^2\,dx = 3 \frac{x^3}{3} = x^3 |_0^1
    = 1^3 - 0^3 = 1
    \]

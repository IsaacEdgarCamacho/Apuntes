...
% =============================================================
% COMANDO PARA REFERENCIAS NORMATIVAS
% =============================================================
\newcommand{\normaref}[1]{%
    \par\noindent\textbf{\textit{Norma:}}~\textit{#1}\par\vspace{0.2em}
}
...
\textit{Comprender a quienes intervienen en el proceso es comprender la lógica misma de la justicia.}
...
El estudio de la materia requiere relacionar dinámicamente los distintos temas que la componen, en tanto el proceso judicial se encuentra formado por distintos subsistemas que se relacionan entre sí.

\section{El proceso judicial y la función jurisdiccional}

El proceso judicial constituye una de las vías posibles para resolver un conflicto; existen además otros métodos alternativos —como el arbitraje, la mediación y la negociación— que también son objeto de estudio del Derecho Procesal.

\begin{definition}{Proceso judicial}
El proceso judicial es un sistema: un conjunto de actos y elementos que se van interrelacionando, que regulan la actividad de la jurisdicción y de las partes, con un objetivo final: la resolución de un conflicto. Esa resolución la dicta el juez en ejercicio de su jurisdicción.
\end{definition}

La jurisdicción es una función que reposa en el Poder Judicial. Los jueces ejercen esa función jurisdiccional en distintos momentos:
...
\begin{itemize}
    ...
    \item \textbf{Ejecutio}: la posibilidad de ejecutar sus decisiones para que se cumplan.
    \item \textbf{Juditio (iudicium)}: el momento máximo de la función jurisdiccional, cuando el juez fija los hechos del proceso, los eleva al plexo normativo sustancial y crea una norma individual —la sentencia— mediante la cual resuelve el conflicto.
\end{itemize}

\begin{definition}{Debido Proceso Legal}
Para que la sentencia sea válida debe haber transitado un Debido Proceso Legal, el cual tiene dos vertientes:
\begin{itemize}
...
Cabe recordar la distinción entre jurisdicción y competencia: todos los jueces ejercen la misma función jurisdiccional, pero dentro de las reglas de competencia creadas por el legislador (por materia, grado y territorio).
...
\normaref{Art. 360 CPCCN --- Audiencia preliminar}
...
\section*{Cuadro Cornell --- Capítulo 1}
\addcontentsline{toc}{section}{Cuadro Cornell --- Capítulo 1}
...
Mediación previa obligatoria; etapa introductoria (demanda y contestación); audiencia preliminar (art.\ 360 CPCCN); etapa probatoria; etapa conclusional (sentencia); etapa recursiva; y, eventualmente, etapa de ejecución. \\

\textbf{¿Qué diferencia hay entre jurisdicción y competencia?} &
Todos los jueces ejercen la misma función jurisdiccional, pero la competencia son las reglas creadas por el legislador (por materia, grado y territorio) que distribuyen esa función entre los distintos jueces. \\

\midrule
\multicolumn{2}{p{\textwidth}}{
\textbf{Resumen:} El proceso es un sistema estructurado en etapas (introductoria, probatoria, conclusional, recursiva y de ejecución). La sentencia, para ser válida, debe respetar el Debido Proceso Legal en su vertiente formal y en su vertiente sustancial.
} \\
\bottomrule
\end{tabularx}
\end{table}
...
El juez es, por excelencia, el sujeto procesal. Es el magistrado que, investido del poder jurisdiccional del Estado, resuelve el conflicto conforme las reglas de la competencia (materia, grado, territorio).

\begin{definition}{Juez natural}
...
\section{Organización judicial en la Ciudad de Buenos Aires}
...
Por encima de ellas, la Corte Suprema de Justicia de la Nación entiende en: (a) competencia originaria (art.\ 117 CN), o (b) cuando hay derecho federal en juego. Existe también la vía del recurso extraordinario federal por sentencia arbitraria, que permite el acceso a la Corte incluso en supuestos de derecho común.

\normaref{Art. 75 inc. 12 CN --- Atribuciones del Congreso / Derecho común vs. derecho federal}
\normaref{Art. 117 CN --- Competencia originaria de la Corte Suprema}

\subsection{Asignación de jueces por sorteo}

El juez que intervendrá en cada proceso se asigna por sorteo, tanto en primera instancia como en las salas de Cámara. La primera vez que el expediente se eleva a la Cámara se sortea la sala interviniente, y por el principio de prevención esa misma sala seguirá interviniendo en todos los recursos de apelación ulteriores del mismo expediente.

\section{Sistema de recusaciones}

\subsection{Recusación sin causa (art. 14 CPCCN)}

\begin{definition}{Recusación}
...
La recusación sin causa (art.\ 14) permite excluir al juez sin expresar ningún fundamento. Sus características son:

\begin{itemize}
    \item Puede ejercerla el actor en la demanda (primera presentación) o el demandado en su primera presentación.
    \item Respecto de jueces de Cámara: se puede ejercer al día siguiente de notificada la primera resolución que dicte la sala.
...
\normaref{Art. 14 CPCCN --- Recusación sin causa}
...
\begin{itemize}
    \item \textbf{Dictar resoluciones} dentro de los plazos legales (providencias simples: en el plazo correspondiente; sentencia definitiva: en el plazo que fija el Código).
    \item \textbf{Asistir personalmente a la audiencia preliminar} del art.\ 360 CPCCN (acto indelegable, en aplicación del principio de inmediación).
\end{itemize}
...
\normaref{Arts. 34, 35, 36 y 37 CPCCN --- Deberes y facultades del juez}
...
\normaref{Arts. 53 y 99 inc. 4 CN --- Nombramiento y remoción de magistrados}

\section{Auxiliares del juez dentro del juzgado}
...
\normaref{Arts. 38 y 39 CPCCN --- Deberes del secretario y prosecretario}

\section{Otras oficinas y organismos auxiliares de la jurisdicción}

Dentro de la organización judicial también existen otros organismos que asisten a la jurisdicción:
...
\section*{Cuadro Cornell --- Capítulo 2}
\addcontentsline{toc}{section}{Cuadro Cornell --- Capítulo 2}
...
El nombramiento corresponde al Presidente con acuerdo de dos tercios del Senado (para la Corte, directamente; para el resto, a partir de una terna vinculante del Consejo de la Magistratura). La remoción se realiza mediante juicio político (jueces de la Corte) o Jurado de Enjuiciamiento (resto de los jueces), por las causales del art.\ 53 CN. \\

\textbf{¿Qué diferencia hay entre primera y segunda instancia en cuanto a la organización?} &
La primera instancia está integrada por jueces unipersonales (civil, comercial, civil y comercial federal, del trabajo); la segunda instancia está integrada por las Cámaras, organizadas en salas de tres jueces (tribunales colegiados), sorteadas por el principio de prevención. \\

\textbf{¿Cuántas veces puede recusarse sin causa?} &
Por única vez en primera instancia y por única vez en Cámara; no procede en juicios ejecutivos, desalojos ni procesos sumarísimos. \\

\textbf{¿Quién resuelve la recusación con causa?} &
La Cámara, cuando el recusado es juez de primera instancia; el mismo tribunal, sin el juez recusado, cuando se trata de un miembro de Cámara o de la Corte. \\

\textbf{¿Qué es una medida de mejor proveer?} &
Una medida destinada a esclarecer la verdad de los hechos controvertidos, dictada por el juez en ejercicio de sus facultades ordenatorias e instructorias (art.\ 36 CPCCN), sin afectar el derecho de defensa ni suplir la negligencia de las partes. \\

\textbf{¿En qué casos interviene el fiscal en un proceso civil?} &
En cuestiones de orden público, como cuando se discute la competencia del tribunal o se plantea la inconstitucionalidad de una norma, emitiendo un dictamen sobre la cuestión. \\

\midrule
\multicolumn{2}{p{\textwidth}}{
\textbf{Resumen:} El órgano jurisdiccional está compuesto por el juez —que debe reunir las notas del juez natural (imparcial, impartial e independiente)— y por sus auxiliares (secretario, prosecretario, cuerpo médico forense, cuerpos interdisciplinarios, oficina de mandamientos y notificaciones). El sistema de recusaciones (sin causa y con causa) preserva ese principio. El Ministerio Público, por su parte, actúa como auxiliar de la jurisdicción en tres vertientes: la legalidad del proceso (fiscal), la protección de menores e incapaces, y la garantía de defensa de pobres y ausentes.
} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{definition}{Parte procesal}
...
Las partes pueden ser personas humanas (físicas) o personas jurídicas. Las personas jurídicas —tanto de derecho público (por ejemplo, el Estado Nacional) como de derecho privado (por ejemplo, sociedades comerciales)— deben actuar a través de sus representantes legales (presidente del directorio en una S.A.; socio gerente en una S.R.L.).

\section{La reconvención}

En la etapa introductoria, al contestar la demanda, el demandado puede a su vez reconvenir (contrademandar) al actor, siempre que haya identidad de objeto (mismo hecho generador) e identidad de sujetos. En ese caso, ambos tienen la doble calidad de actor y de demandado:
...
\normaref{Art. 24 CCyC --- Personas incapaces}
\normaref{Arts. 152 y 153 CCyC --- Domicilio de personas jurídicas}

\subsection{Poder, mandato y representación}

Son tres conceptos distintos que deben diferenciarse:
...
\begin{itemize}
    ...
    \item \textbf{Mandato}: el contrato (regulado a partir del art.\ 1.319 CCyC y siguientes) que rige la relación entre el cliente y su abogado. En este acto sí está presente el apoderado. Puede haber mandato sin representación.
...
\normaref{Art. 1.319 y ss. CCyC --- Contrato de mandato}
\normaref{Art. 56 CPCCN --- Obligatoriedad de la asistencia letrada}
...
En la etapa introductoria, la parte que advierta alguno de estos defectos puede oponerlos como excepciones previas (art.\ 347 CPCCN):
...
\normaref{Art. 347 CPCCN --- Excepciones previas (incisos 2 y 3)}
...
\normaref{Art. 40 CPCCN --- Domicilio procesal}
\normaref{Art. 73 CCyC --- Domicilio de las personas físicas}
\normaref{Arts. 152 y 153 CCyC --- Domicilio de las personas jurídicas}
\normaref{Art. 133 CPCCN --- Notificación por ministerio de la ley}

\section*{Cuadro Cornell --- Capítulo 3}
\addcontentsline{toc}{section}{Cuadro Cornell --- Capítulo 3}
...
Siempre dos: actor y demandado, con independencia de la multiplicidad de sujetos que integren cada polo (litisconsorcio) o de la doble calidad que asuman en caso de reconvención. \\

\textbf{¿Qué es la reconvención?} &
La contrademanda que el demandado dirige contra el actor al contestar la demanda, cuando hay identidad de objeto y de sujetos; genera la doble calidad de actor-reconvenido y demandado-reconviniente. \\

\textbf{¿Puede ser parte alguien sin legitimación?} &
Sí. La falta de legitimación no invalida la calidad de parte; las partes lo son durante todo el proceso, pero la falta de legitimación determina el rechazo de la pretensión en la sentencia. \\

\textbf{¿En qué momento influye la falta de legitimación?} &
Puede resolverse como excepción previa en la etapa introductoria (art.\ 347 inc.\ 3 CPCCN) o diferirse al momento de la sentencia cuando su resolución requiera producción de prueba. \\

\textbf{¿Qué diferencia hay entre poder, mandato y representación?} &
El poder es el instrumento que otorga la facultad de representar (interviene el poderdante y el escribano); el mandato es el contrato que rige la relación entre cliente y abogado (interviene el apoderado); la representación puede existir con o sin mandato. \\

\textbf{¿Qué pasa si no se constituye domicilio procesal?} &
Todas las notificaciones que debían cursarse a ese domicilio quedan notificadas automáticamente por ministerio de la ley los martes o viernes siguientes a la resolución (art.\ 133 CPCCN), sin necesidad de cédula. \\

\midrule
\multicolumn{2}{p{\textwidth}}{
\textbf{Resumen:} Las partes —actor y demandado— son siempre dos personas (humanas o jurídicas), asistidas obligatoriamente por un letrado (art.\ 56 CPCCN). Deben distinguirse la capacidad civil (aptitud para ser titular de derechos), la legitimación en la causa (titularidad de la relación jurídica sustancial) y la representación (legal o convencional), cuya ausencia puede oponerse como excepción previa (art.\ 347 CPCCN). Además, deben denunciar domicilio real y constituir domicilio procesal, bajo apercibimiento de notificación automática por ministerio de la ley.
} \\
\bottomrule
\end{tabularx}
\end{table}
...
% CAPÍTULO 4
% =============================================================
\chapter{Sistemas procesales, cargas y litisconsorcio}
...
\subsection{Sistema de preclusión y sistema de unicidad}
...
\textbf{Sistema de unicidad} (alternativo, no elegido por el legislador): existe un único período en el que pueden ejercerse todas las facultades y cargas, sin cierre de etapas.

\subsection{Sistema dispositivo y sistema de oficiosidad}

\textbf{Sistema dispositivo}: en el proceso civil y comercial son las partes quienes tienen a cargo el impulso del proceso. La inactividad procesal puede conducir a la caducidad de la instancia.

\textbf{Sistema de oficiosidad} (propio del proceso laboral): el impulso lo lleva la jurisdicción, lo que explica que en el proceso laboral no exista la figura de la caducidad de instancia.

\begin{note}
La caducidad de instancia no interrumpe la prescripción: si el proceso caduca, debe reiniciarse siempre que la acción aún no haya prescripto.
\end{note}

\section{Cargas, deberes y obligaciones procesales}

\begin{definition}{Carga procesal}
Facultad que tiene la parte de realizar determinados actos en ciertos momentos del proceso. Se la llama \textit{``imperativo del propio interés''} porque su no ejercicio no trae aparejada ninguna sanción; a lo sumo puede generar un beneficio para el otro litigante.
\end{definition}

\begin{example}{Carga procesal}
El demandado puede optar por no contestar la demanda dentro del plazo de quince días en el proceso ordinario. No hay sanción por ello. Sin embargo, el no ejercicio de esa carga genera un beneficio al actor: el juez tendrá una cierta presunción de verdad respecto de los hechos invocados por el actor, porque el demandado no los negó ni dio su propia versión.
\end{example}
...
\subsection{Litisconsorcio necesario (obligatorio)}

\begin{definition}{Litisconsorcio necesario}
...
\begin{example}{Litisconsorcio necesario}
Cuatro personas son condóminas de un inmueble (25\,\% cada una). Una de ellas quiere vender y promueve un proceso de división de condominio solo contra dos de las condóminas que están ocupando el inmueble, omitiendo a la cuarta copropietaria. Para que la sentencia de división sea útil y oponible a todos, es indispensable integrar la litis con todos los condóminos. El juez puede —de oficio o a pedido de parte— dar intervención al litisconsorte ausente.
\end{example}

\subsection{Litisconsorcio facultativo o voluntario}

\begin{definition}{Litisconsorcio facultativo}
...
\begin{example}{Litisconsorcio facultativo}
...
\section{Acumulación de procesos}
...
\section*{Cuadro Cornell --- Capítulo 4}
\addcontentsline{toc}{section}{Cuadro Cornell --- Capítulo 4}
...
Obliga a reiniciar el proceso si la inactividad procesal se prolonga en el sistema dispositivo; no interrumpe la prescripción, por lo que el proceso puede reiniciarse siempre que la acción no haya prescripto. \\

\textbf{¿Por qué no existe caducidad en el proceso laboral?} &
Porque el proceso laboral se rige por el sistema de oficiosidad: el impulso del proceso está a cargo de la jurisdicción y no de las partes. \\

\textbf{¿Cuál es la diferencia entre una carga y un deber?} &
La carga es un ``imperativo del propio interés'' cuyo no ejercicio no genera sanción, aunque puede beneficiar al adversario; el deber es un imperativo legal cuyo incumplimiento sí genera sanción. \\

\textbf{¿Qué consecuencias tiene no contestar la demanda?} &
No genera sanción directa, pero el juez podrá tener una presunción de verdad respecto de los hechos invocados por el actor, al no haber sido negados por el demandado. \\

\textbf{¿Cuándo es obligatorio el litisconsorcio?} &
Cuando la sentencia solo puede ser útil y eficaz si intervienen todos los litisconsortes necesarios (litisconsorcio necesario); de lo contrario, el juez puede denegar el avance del proceso hasta integrar la litis. \\

\textbf{¿Cuál es el criterio para acumular procesos?} &
Los procesos se acumulan ante el juez que previno —aquel en cuyo proceso se notificó primero la demanda al accionado— para evitar sentencias contradictorias sobre el mismo hecho. \\

\midrule
\multicolumn{2}{p{\textwidth}}{
\textbf{Resumen:} El proceso civil y comercial es dispositivo (las partes impulsan el proceso, bajo apercibimiento de caducidad de instancia) y preclusivo (avanza por etapas cerradas). Las cargas se distinguen de los deberes por la ausencia de sanción ante su incumplimiento. El litisconsorcio puede ser necesario (indispensable para que la sentencia sea útil) o facultativo (por conexidad de pretensiones), y los procesos conexos pueden acumularse ante el juez que previno para evitar sentencias contradictorias.
} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{definition}{Proceso colectivo}
Existe un hecho generador común que puede afectar a un número indeterminado e indefinido de personas. Su regulación no se encuentra en el CPCCN ni en ninguna ley específica; debe buscarse en la Constitución Nacional y en los precedentes de la Corte Suprema.
\end{definition}
...
\begin{itemize}
    ...
    \item Art.\ 43, 2.\textsuperscript{o} párrafo, CN: podrán interponer la acción de amparo contra cualquier forma de discriminación y en lo relativo a los derechos que protegen al ambiente, a la competencia, al usuario, al consumidor, así como a los derechos de incidencia colectiva en general, el afectado, el defensor del pueblo y las asociaciones que propendan a esos fines.
\end{itemize}

\normaref{Arts. 41, 42 y 43 CN --- Derechos de incidencia colectiva}
...
\begin{itemize}
    ...
    \item \textbf{Voluntaria} (arts.\ 90 y 91 CPCCN): el tercero se presenta espontáneamente. A su vez puede ser: (a) litisconsorcial, o (b) adhesiva.
\end{itemize}

\normaref{Arts. 90, 91, 94 y 96 CPCCN --- Terceros intervinientes}
...
\begin{example}{Intervención obligada --- citación del deudor principal}
...
\subsection{Intervención voluntaria litisconsorcial (arts. 90 y 91 CPCCN)}
...
\begin{example}{Tercero litisconsorcial --- socio}
...
\begin{example}{Tercero litisconsorcial --- copropietario}
...
\section*{Cuadro Cornell --- Capítulo 5}
\addcontentsline{toc}{section}{Cuadro Cornell --- Capítulo 5}
...
En el litisconsorcio cada parte tiene un derecho subjetivo individual dentro de una relación jurídica determinada y todas las partes son individualizables; en el proceso colectivo existe un hecho generador común que afecta a un número indeterminado e indefinido de personas, sin regulación en el CPCCN. \\

\textbf{¿Qué son los derechos individuales homogéneos?} &
Aquellos en los que el mismo hecho generador afecta a una cantidad numerosa pero determinable de personas, cada una resarcible individualmente según el perjuicio concreto sufrido, y unidas por una causa común. \\

\textbf{¿Cuándo puede ejecutarse la sentencia contra un tercero?} &
Cuando el tercero fue citado obligadamente (arts.\ 94 y 96 CPCCN) y la controversia le es común; el art.\ 96 CPCCN permite ejecutar la sentencia contra él como si fuera parte, salvo que invoque defensas ajenas al marco de debate del proceso. \\

\textbf{¿Qué diferencia hay entre intervención litisconsorcial y adhesiva?} &
El tercero litisconsorcial acredita legitimación sustancial propia y actúa con rol autónomo, con las mismas facultades que una parte originaria; el tercero adhesivo solo acredita un interés propio conexo, carece de rol autónomo y la sentencia no lo alcanza directamente. \\

\midrule
\multicolumn{2}{p{\textwidth}}{
\textbf{Resumen:} El proceso colectivo, fundado en los arts.\ 41 a 43 CN y en los precedentes de la Corte Suprema, protege derechos difusos (bien indivisible) o individuales homogéneos (causa común, resarcimiento individual), y se distingue del litisconsorcio por la indeterminación de sus titulares. Los terceros intervinientes actúan por un interés propio: los obligados son citados por las partes y pueden quedar alcanzados por la sentencia; los voluntarios pueden ser litisconsorciales (rol autónomo) o adhesivos (rol limitado, sin alcance directo de la sentencia).
} \\
\bottomrule
\end{tabularx}
\end{table}

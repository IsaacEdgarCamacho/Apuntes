...
\item %1)
		\begin{enumerate}[i)]
			\item %a 
				$  \underbrace{(a \cdot b) | c}_{Hipótesis} $ $\Rightarrow$ $\underbrace{(a | c)}_{Tésis1}$  y $\underbrace{(b | c)}_{Tésis2}$, Veámos qué significa la hipótesis, si $(a \cdot b)$ divide a c entonces, $ c = k \cdot (a\cdot b)$ como el producto en los $\mathbb{Z}$ es asociativo, 
	$ c = \underbrace{(k \cdot a )}_{t} \cdot b = t \cdot b $, y esto significa que b divide a c ($b |c$), de igual manéra sabiendo que el producto en $\mathbb{Z}$ es conmutativo y asociativo $ c = \overbrace{(k \cdot b)}^r \cdot a$, por lo tanto $c = r \cdot a \Rightarrow \ a|c$
			\item %b
		\end{enumerate}
